...
surity1=np.random.uniform(0.8,1.2)
surity2=np.random.uniform(-1.1,0.25)
...
#optimize
disc_optimizer =  tf.train.GradientDescentOptimizer(1.1*1e-3).minimize(d_loss,var_list=discriminator.trainable_weights)
...
gen_optimizer = tf.train.AdamOptimizer(1.2*1e-4).minimize(g_loss,var_list=generator.trainable_weights)
...
for epoch in range(100001):
    ...
    if epoch %50==0:
        if epoch>249 and ((gen_loss<1.3*best_loss and gen_loss<4) or gen_loss<1.5):
            generator.save_weights("best_weights2.h5")
            best_loss=gen_loss
        if gen_loss>4.9:
            generator.load_weights("best_weights2.h5")
    
    if epoch%20000==0 and epoch!=0:
        generator.save_weights("generator_weights2_"+str(epoch)+".h5")
        discriminator.save_weights("discriminator_weights2_"+str(epoch)+".h5")
