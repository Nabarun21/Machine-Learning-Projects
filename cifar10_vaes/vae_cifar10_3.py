...
x=Conv2D(64,4,padding='same',kernel_initializer='he_uniform',use_bias=False,strides=2)(inputs)
x=BatchNormalization()(x)
x=ELU(alpha=0.1)(x)
#x=Dropout(0.2)(x)
...
print(np.shape(kl_div))
vae_loss=K.mean(reconstruction_loss + beta*kl_div)

#   return total_loss
vae.add_loss(vae_loss)
my_opt = optimizers.Adam(lr=0.001, decay=1e-5)
...
vae.fit(x_train,None,epochs=501,batch_size=batch_size,validation_data=(x_test, None),callbacks=[model_save_callback,warmup_callback],verbose=2)
