...
x=Conv2D(64,4,padding='same',kernel_initializer='he_uniform',use_bias=False)(inputs)
...
x=Conv2DTranspose(128,4,padding='same',kernel_initializer='he_uniform',use_bias=False)(x)
x=BatchNormalization()(x)
x=ELU(alpha=0.1)(x)
...
my_opt = optimizers.Adam(lr=0.003, decay=1.01e-4)
...
def save_model_weights(epoch):
    if epoch==20 or epoch%100==0:
        vae.save_weights('cifar_vae2_weights'+str(epoch)+'.h5')

def warmup(epoch):
    new_beta=(epoch+0.01)/7. if epoch<10 else 1
...
vae.fit(x_train,None,epochs=601,batch_size=batch_size,validation_data=(x_test, None),callbacks=[model_save_callback,warmup_callback],verbose=2)
