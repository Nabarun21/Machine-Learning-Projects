...
def generator(z, is_training,b_size):
    ...
    h_1=ops.deconv2d(h_in_z,output_shape=[b_size,8,8,512],k_h=5,k_w=5,d_h=2, d_w=2,name="g_deconv_1")
    ...
    h_2=ops.deconv2d(h_1_z_dr,output_shape=[b_size,16,16,256],k_h=5,k_w=5,d_h=2, d_w=2,name="g_deconv_2")
    ...
    h_3=ops.deconv2d(h_2_z_dr,output_shape=[b_size,32,32,128],k_h=5,k_w=5,d_h=2, d_w=2,name="g_deconv_3")
    ...
    h_out = ops.deconv2d(h_3_z, [b_size, IMAGE_SIZE, IMAGE_SIZE, N_CHANNELS],
            name='g_out')
...
with tf.variable_scope("G") as scope:
    ...
    g_batch_size=tf.placeholder(tf.int32, name='g_batch_size')
    G = generator(z, is_training,g_batch_size)
    
with tf.variable_scope('D') as scope:
    images = tf.placeholder(tf.float32, shape=[None, IMAGE_SIZE, IMAGE_SIZE, N_CHANNELS])

    r_batch_size=tf.placeholder(tf.int32, name='r_batch_size')
...
d_optim_real = tf.train.AdamOptimizer(LEARNING_RATE, beta1=BETA).minimize(d_loss_real*surity1, var_list=d_vars)
d_optim_fake = tf.train.AdamOptimizer(LEARNING_RATE, beta1=BETA).minimize(d_loss_fake*surity2, var_list=d_vars)
...
def train(sess, load_dir=None, save_frequency=100, sample_frequency=100, sample_dir='sample_faces2',
          save_dir='checkpoint2', max_to_keep=30, model_name='dcgan.model3',
    ...
    for epoch in range(n_epochs):
        ...
        for idx in range(batch_idxs*2):
           ...
           # batch_images = np.array(batch).astype(np.float32)
            ...
            # Update D network
            sess.run(d_optim_real, feed_dict={images: batch_images,is_training: True})
            sess.run(d_optim_fake, feed_dict={z: batch_z,g_batch_size:BATCH_SIZE,is_training: True})

            # Update G network
            if epoch>15:n_generator_update=1
            for _ in range(n_generator_update):
                sess.run(g_optim,
                    feed_dict={z: batch_z,g_batch_size:BATCH_SIZE,is_training: True})


            errD_fake = d_loss_fake.eval({z: batch_z,g_batch_size:BATCH_SIZE, is_training: False})
            errD_real = d_loss_real.eval({images: batch_images,is_training: False})
            errG = g_loss.eval({z: batch_z,g_batch_size:BATCH_SIZE,is_training: False})
            ...
            if np.mod(counter, sample_frequency) == 1:
                samples = sess.run(G, feed_dict={z: batch_z,g_batch_size:BATCH_SIZE,is_training: False} )
...
#    train(sess,load_dir='checkpoint2',save_dir='checkpoint2',sample_frequency=500,save_frequency=3000,latest_fname="dcgan.model3_9")
    train(sess,save_dir='checkpoint2',sample_frequency=500,save_frequency=2000)
