...
def plot_history(history):
    ...
    plt.legend(['train', 'val'], loc='upper left')
    plt.show()
def plot_history2(save_name):
    with open('classification_train_acc_'+save_name+'.txt', "rb") as fp: 
        classification_train_acc_list=pickle.load(fp)
    with open('classification_val_acc_'+save_name+'.txt', "rb") as fp: 
        classification_val_acc_list=pickle.load(fp)
    with open('classification_train_loss_'+save_name+'.txt', "rb") as fp: 
        classification_train_loss_list=pickle.load(fp)
    with open('classification_val_loss_'+save_name+'.txt', "rb") as fp: 
        classification_val_loss_list=pickle.load(fp)

    # summarize history for accuracy
    plt.plot(classification_train_acc_list)
    plt.plot(classification_val_acc_list)
    plt.title('model accuracy')
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
    plt.legend(['train', 'val'], loc='upper left')
    plt.show()
    # summarize history for loss
    plt.plot( classification_train_loss_list)
    plt.plot(classification_val_loss_list)
    plt.title('model loss')
    plt.ylabel('loss')
    plt.xlabel('epoch')
    plt.legend(['train', 'val'], loc='upper left')
...
def save_hist(history,save_name):
    with open('classification_train_acc_'+save_name+'.txt', "wb") as fp: 
        pickle.dump(history.history['acc'], fp)
    with open('classification_val_acc_'+save_name+'.txt', "wb") as fp: 
        pickle.dump(history.history['val_acc'], fp)
    with open('classification_train_loss_'+save_name+'.txt', "wb") as fp: 
        pickle.dump(history.history['loss'], fp)
    with open('classification_val_loss_'+save_name+'.txt', "wb") as fp: 
