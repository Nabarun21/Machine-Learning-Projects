...
class decoder:
    ...
    lstm_fw = tf.nn.rnn_cell.LSTMCell(LSTM_UNITS)

    lstm_bw = tf.nn.rnn_cell.LSTMCell(LSTM_UNITS)
    
    lstm_fw_2 = tf.nn.rnn_cell.LSTMCell(LSTM_UNITS)

    lstm_bw_2 = tf.nn.rnn_cell.LSTMCell(LSTM_UNITS)

    lstm_fw_3 = tf.nn.rnn_cell.LSTMCell(LSTM_UNITS)

    lstm_bw_3 = tf.nn.rnn_cell.LSTMCell(LSTM_UNITS)
    ...
    hidden_states, _ = tf.contrib.rnn.stack_bidirectional_dynamic_rnn ([lstm_fw,lstm_fw_2,lstm_fw_3],[lstm_bw,lstm_bw_2,lstm_bw_3],word_embeds,dtype='float32',
                                                                       initial_states_fw=[tf.nn.rnn_cell.LSTMStateTuple(c0, h0),tf.nn.rnn_cell.LSTMStateTuple(c0, h0),tf.nn.rnn_cell.LSTMStateTuple(c0, h0)])
    ...
    flat_ground_truth_fw =tf.reshape(sentences[:,1:],[-1]) 
    flat_ground_truth=tf.concat([flat_ground_truth_fw,flat_ground_truth_fw],0)
...
for epoch in range(n_epochs):
    ...
    if epoch%5==0:
        saver.save(s, os.path.abspath("weights_stack_bidir_{}".format(epoch)))
...
saver.save(s, os.path.abspath("weights_stack_bidir"))
