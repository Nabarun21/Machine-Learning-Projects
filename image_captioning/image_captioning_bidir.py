...
class decoder:
    ...
    hidden_states_uncon, _ = tf.nn.bidirectional_dynamic_rnn(lstm_fw,lstm_bw,word_embeds,dtype='float32',
                                         initial_state_fw=tf.nn.rnn_cell.LSTMStateTuple(c0, h0))

    # now we need to calculate token logits for all the hidden states
    hidden_states=tf.concat(hidden_states_uncon,2)
    # first, we reshape `hidden_states` to [-1, LSTM_UNITS]
    flat_hidden_states = tf.reshape(hidden_states,shape=[-1,2*LSTM_UNITS])### YOUR CODE HERE ###
...
for epoch in range(n_epochs):
    ...
    if epoch%5==0:
        saver.save(s, os.path.abspath("weights_bidir_{}".format(epoch)))
...
saver.save(s, os.path.abspath("weights_bidir"))
