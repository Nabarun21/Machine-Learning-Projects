...
class decoder:
    ...
    img_embed_bottleneck_to_h0 = L.Dense(LSTM_UNITS,
                                         ...
                                         activation='elu')
    # image embedding bottleneck -> lstm initial state
    img_embed_bottleneck_to_h0_2 = L.Dense(2*LSTM_UNITS,
                                         input_shape=(None, IMG_EMBED_BOTTLENECK),
                                         activation='elu')

    # image embedding bottleneck -> lstm initial state
    img_embed_bottleneck_to_h0_3 = L.Dense(1.5*LSTM_UNITS,
                                         input_shape=(None, IMG_EMBED_BOTTLENECK),
                                         activation='elu')
    ...
    lstm_1 = tf.nn.rnn_cell.LSTMCell(LSTM_UNITS)
    lstm_2 = tf.nn.rnn_cell.LSTMCell(2*LSTM_UNITS)
    lstm_3 = tf.nn.rnn_cell.LSTMCell(1.5*LSTM_UNITS)
    
    # we use bottleneck here to reduce model complexity
    # lstm output -> logits bottleneck
    token_logits_bottleneck = L.Dense(LOGIT_BOTTLENECK, 
                                      input_shape=(None, int(1.5*LSTM_UNITS)),
                                      activation="elu")
    ...
    c0_2 = h0_2 = img_embed_bottleneck_to_h0_2(img_embed_to_bottleneck(img_embeds))### YOUR CODE HERE ###
    c0_3 = h0_3 = img_embed_bottleneck_to_h0_3(img_embed_to_bottleneck(img_embeds))### YOUR CODE HERE ###
    ...
    with tf.variable_scope("foo") as scope:
        hidden_states_1, _ = tf.nn.dynamic_rnn(lstm_1, word_embeds,
                                           initial_state=tf.nn.rnn_cell.LSTMStateTuple(c0, h0))

    with tf.variable_scope("bar") as scope:#        scope.reuse_variables()
        hidden_states_2, _ = tf.nn.dynamic_rnn(lstm_2, hidden_states_1,initial_state=tf.nn.rnn_cell.LSTMStateTuple(c0_2, h0_2))

    with tf.variable_scope("koo") as scope:#        scope.reuse_variables()
        hidden_states_3, _ = tf.nn.dynamic_rnn(lstm_3, hidden_states_2,
                                               initial_state=tf.nn.rnn_cell.LSTMStateTuple(c0_3, h0_3))

    # now we need to calculate token logits for all the hidden states
    
    # first, we reshape `hidden_states` to [-1, LSTM_UNITS]
    flat_hidden_states = tf.reshape(hidden_states_3,shape=[-1,int(1.5*LSTM_UNITS)])### YOUR CODE HERE ###
...
for epoch in range(n_epochs):
    ...
    if epoch%5==0:
        saver.save(s, os.path.abspath("weights_stacked_{}".format(epoch)))
...
saver.save(s, os.path.abspath("weights_stacked"))
