...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %forward prop, for hiddden layer
         
  ret.input_to_hid = model.input_to_hid * 0;
  ret.hid_to_class = model.hid_to_class * 0;
  
  hidden_layer_logit=model.input_to_hid*data.inputs;
  hidden_layer=logistic(hidden_layer_logit);
  output_layer_logit=model.hid_to_class*hidden_layer;
  
  
  class_normalizer = log_sum_exp_over_rows( output_layer_logit); % log(sum(exp of class_input)) is what we subtract to get properly normalized log class probabilities. size: <1> by <number of data cases>
  log_class_prob = output_layer_logit - repmat(class_normalizer, [size(output_layer_logit, 1), 1]); % log of probability of each class. size: <number of classes, i.e. 10> by <number of data cases>
  class_prob = exp(log_class_prob); % probability of each class. Each column (i.e. each case) sums to 1. size: <number of classes, i.e. 10> by <number of data cases>
  
  
  target_pred_diff=class_prob-data.targets;
  
  hidd_layer_logistic_slope_term=hidden_layer.*(1-hidden_layer);
  
  need_for_later=model.hid_to_class'*target_pred_diff;
  
  %calculated hid_to_class gradients
  for input_index=1:size(data.inputs,2),
      curr_hid_to_class_grad=target_pred_diff(:,input_index)*hidden_layer(:,input_index)';
      curr_input_to_hid_grad_part=hidd_layer_logistic_slope_term(:,input_index)*data.inputs(:,input_index)';
      curr_input_to_hid_grad_part=bsxfun(@times,need_for_later(:,input_index),curr_input_to_hid_grad_part);      
      
      
      ret.hid_to_class=ret.hid_to_class+curr_hid_to_class_grad;
      ret.input_to_hid=ret.input_to_hid+curr_input_to_hid_grad_part;
  end 
  
  ret.hid_to_class=ret.hid_to_class/size(data.inputs,2)+model.hid_to_class*wd_coefficient;
  ret.input_to_hid=ret.input_to_hid/size(data.inputs,2)+model.input_to_hid*wd_coefficient;
