...
import tqdm
class Classifier():
    def __init__(self, nn_model, layer = 'fc6',k=15):
        """Your implementation"""
    ...
    def fit(self, train_imgs, train_labels):
        ...
        for key,value in train_imgs.items():
            ...
            if prep_image.shape!=train_imgs[key].shape:
                ...
                train_array_x.append(prep_image)
                train_array_y.append(train_labels[key])
        
        train_array_x=np.array(train_array_x)
        train_array_y=np.array(train_array_y)

        train_images_reshape=np.reshape(train_array_x,newshape=(-1,224,224,3))
        train_img_features=self.feature_extraction_model.predict(train_images_reshape)
        
        print(train_img_features.shape)
#        train_array_x=np.reshape(train_array_x,newshape=(-1,4096))
        #print(train_array_x.shape)
        self.clf.fit(train_img_features,train_array_y)
    ...
    def classify_videos(self, test_video):
        """Your implementation"""
        ret_val={}
        for key,image_list in test_video.items():
            curr_images=[]
            for image in image_list[:10]:
                prep_image=preprocess_img(image)
                if prep_image.shape!=image.shape:#if no face found,skip
                    prep_image=cv.resize(prep_image,(224,224))
                    curr_images.append(prep_image)
            
            prep_images_reshape=np.reshape(curr_images,newshape=(-1,224,224,3))
            test_img_features=self.feature_extraction_model.predict(prep_images_reshape)
            predicted_labels=self.clf.predict(test_img_features)
...
def check_test(output, gt):
    ...
    for k, v in gt.items():
        ...
        if output[k] == v:
            ...
            print("It was ",v," but we predicted ",output[k])
